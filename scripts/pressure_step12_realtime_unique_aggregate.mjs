...
activeEvents.sort(
  (a, b) => Number(a[0]) - Number(b[0]) || Number(b[1]) - Number(a[1]),
);
let active = 0;
...
console.log("STEP12_REALTIME_LEVEL5000_UNIQUE " + JSON.stringify(result));
fs.writeFileSync(
  "step12-realtime-level5000-summary.json",
