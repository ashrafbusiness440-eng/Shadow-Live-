...
export const controlApi=onRequest({region:"us-central1"},async(req,res)=>{
 ...
 try{
  if(req.method==="GET"&&req.path.endsWith("/v1/control/health")){
   ...
   res.status(coreReady?200:503).set("Cache-Control","no-store").json({ok:coreReady,environment,version:"1",api:true,firestore:checks.firestore,auth:checks.auth,audit:checks.firestore,ledger:checks.firestore,financialWritesEnabled:coreReady&&privilegedEnabled,roleMutationsEnabled:coreReady&&privilegedEnabled});
   return;
  }
