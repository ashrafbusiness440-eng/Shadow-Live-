...
class FirebaseService {
 ...
 Future<String> ensurePublicId(String userId)async{
  ...
  for(var attempt=0;attempt<16;attempt++){final id=(10000000+random.nextInt(90000000)).toString();final idRef=_firestore.collection('public_ids').doc(id);try{return await _firestore.runTransaction<String>((tx)async{final userSnap=await tx.get(userRef);final already=userSnap.data()?['publicId']?.toString();if(already!=null&&already.isNotEmpty)return already;final idSnap=await tx.get(idRef);if(idSnap.exists)throw StateError('collision');tx.set(idRef,{'uid':userId,'createdAt':FieldValue.serverTimestamp()});tx.set(userRef,{'publicId':id,'updatedAt':FieldValue.serverTimestamp()},SetOptions(merge:true));return id;});}catch(e){if(e is StateError)continue;rethrow;}}
 ...
 Future<void> createUserProfile(String userId,Map<String,dynamic> data)async{try{final ref=_firestore.collection('users').doc(userId);final snap=await ref.get();if(!snap.exists){await ref.set({..._profileDefaults(userId),...data,'createdAt':FieldValue.serverTimestamp(),'updatedAt':FieldValue.serverTimestamp()});}else{await ref.set({...data,'updatedAt':FieldValue.serverTimestamp()},SetOptions(merge:true));}await ensurePublicId(userId);}catch(e){throw Exception('Failed to create user profile: $e');}}
 Future<Map<String,dynamic>?> getUserProfile(String userId)async{try{final doc=await _firestore.collection('users').doc(userId).get();if(!doc.exists)return null;final data=doc.data();if(data!=null&&(data['publicId']==null||data['publicId'].toString().isEmpty))data['publicId']=await ensurePublicId(userId);return data;}catch(e){throw Exception('Failed to get user profile: $e');}}
 Future<void> updateUserProfile(String userId,Map<String,dynamic> data)async{try{await _firestore.collection('users').doc(userId).set({...data,'updatedAt':FieldValue.serverTimestamp()},SetOptions(merge:true));}catch(e){throw Exception('Failed to update user profile: $e');}}
