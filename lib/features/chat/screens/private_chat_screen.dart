...
class _PrivateChatScreenState extends State<PrivateChatScreen> {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      batch.set(_conversation, {'participants': [_uid, widget.otherUid], 'lastMessage': text, 'lastSenderId': _uid, 'updatedAt': now}, SetOptions(merge: true));
      batch.set(_conversation.collection('messages').doc(), {'senderId': _uid, 'text': text, 'type': 'text', 'createdAt': now});
      await batch.commit();
    } catch (_) {
  ...
  Widget build(BuildContext context) => Directionality(
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
        ...
        titleSpacing: 0,
        title: Row(children: [
          CircleAvatar(radius: 18, backgroundColor: const Color(0xFF25183F), backgroundImage: widget.otherPhoto.isNotEmpty ? NetworkImage(widget.otherPhoto) : null, child: widget.otherPhoto.isEmpty ? const Icon(Icons.person_rounded, color: Color(0xFFFFD54A), size: 20) : null),
      ...
      body: Column(children: [
                ...
                itemBuilder: (_, i) {
                  ...
                  return Align(
                    ...
                    child: Container(
                      ...
                      child: Text('${data['text'] ?? ''}', style: const TextStyle(color: Colors.white, height: 1.35)),
                    ),
                  );
                },
              );
        ...
        SafeArea(top: false, child: Container(
          ...
          child: Row(children: [
            Expanded(child: TextField(controller: _controller, minLines: 1, maxLines: 5, textInputAction: TextInputAction.newline, style: const TextStyle(color: Colors.white), decoration: InputDecoration(hintText: 'اكتب رسالة...', hintStyle: const TextStyle(color: Colors.white38), filled: true, fillColor: const Color(0xFF151925), border: OutlineInputBorder(borderRadius: BorderRadius.circular(22), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10)))),
            const SizedBox(width: 8),
            IconButton.filled(onPressed: _sending ? null : _send, style: IconButton.styleFrom(backgroundColor: const Color(0xFF7B2DFF), foregroundColor: Colors.white), icon: _sending ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(Icons.send_rounded)),
          ]),
        )),
      ]),
    ),
