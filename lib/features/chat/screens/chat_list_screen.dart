...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  Future<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _searchUsers(String query) async {
    ...
    try {
      ...
      if (targetUid != null && targetUid != uid) {
        ...
        for (final doc in all.docs) { found[doc.id] = doc; }
      }
    } catch (_) {}
    try {
      final users = await FirebaseFirestore.instance.collection('users').orderBy('displayName').startAt([q]).endAt(['$q\uf8ff']).limit(20).get();
      for (final doc in users.docs) { if (doc.id != uid) found[doc.id] = doc; }
    } catch (_) {}
  ...
  Future<List<QueryDocumentSnapshot<Map<String, dynamic>>>> _suggestedUsers() async {
    try {
      final snap = await FirebaseFirestore.instance.collection('users').orderBy('createdAt', descending: true).limit(20).get();
      ...
      try {
        ...
        final docs = snap.docs.where((d) => d.id != uid).toList();
        docs.sort((a, b) {
  ...
  Widget _userTile(BuildContext sheetContext, QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    return ListTile(
      ...
      leading: CircleAvatar(backgroundColor: const Color(0xFF25183F), backgroundImage: provider, child: provider == null ? const Icon(Icons.person, color: Color(0xFFFFD54A)) : null),
              ...
              child: SizedBox(
                ...
                child: Column(children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(prefixIcon: const Icon(Icons.search, color: Color(0xFFFFD54A)), hintText: 'ابحث بالاسم أو ID', hintStyle: const TextStyle(color: Colors.white38), filled: true, fillColor: const Color(0xFF181C29), border: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: BorderSide.none)),
                    onChanged: (value) async {
                      final version = ++searchVersion;
                      final q = value.trim();
                      if (q.length < 2) { setSheetState(() { loading = false; results = []; }); return; }
                      setSheetState(() => loading = true);
                      final next = await _searchUsers(q);
                      if (!context.mounted || version != searchVersion) return;
                      setSheetState(() { results = next; loading = false; });
                    },
                  ),
                  const SizedBox(height: 14),
                  Expanded(child: loading
                    ...
                    : searching
                      ? (results.isEmpty ? const Center(child: Text('لا توجد نتائج', style: TextStyle(color: Colors.white54))) : ListView.builder(itemCount: results.length, itemBuilder: (_, i) => _userTile(sheetContext, results[i])))
    ...
    child: Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        Padding(padding: const EdgeInsets.fromLTRB(18,18,18,12), child: Row(children: [
          const Icon(Icons.chat_bubble_rounded, color: Color(0xFFFFD54A)), const SizedBox(width:10),
        ...
        Expanded(child: StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
          ...
          builder:(context,snapshot){
            ...
            final docs=[...snapshot.data!.docs]..sort((a,b)=>((b.data()['updatedAt'] as Timestamp?)?.millisecondsSinceEpoch??0).compareTo((a.data()['updatedAt'] as Timestamp?)?.millisecondsSinceEpoch??0));
            if(docs.isEmpty)return _state(Icons.forum_outlined,'لا توجد محادثات بعد\nاضغط + لبدء محادثة');
            return ListView.separated(padding:const EdgeInsets.fromLTRB(12,4,12,20),itemCount:docs.length,separatorBuilder:(_,__)=>const SizedBox(height:8),itemBuilder:(_,index){
              final doc=docs[index]; final data=doc.data(); final participants=List<String>.from(data['participants']??const[]); final other=participants.firstWhere((id)=>id!=uid,orElse:()=>''); final unread=((data['unreadCounts'] as Map?)?[uid] as num?)?.toInt()??0;
...
class _ConversationTile extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:otherUid.isEmpty?null:FirebaseFirestore.instance.collection('users').doc(otherUid).get(),builder:(context,snapshot){
    final user=snapshot.data?.data()??const<String,dynamic>{};final name='${user['displayName']??user['name']??'مستخدم Shadow Live'}';final photo='${user['profileImageUrl']??user['photoUrl']??user['avatarUrl']??''}';final asset='${user['profileAvatarAsset']??''}';ImageProvider? provider;if(photo.isNotEmpty)provider=NetworkImage(photo);if(photo.isEmpty&&asset.isNotEmpty)provider=AssetImage(asset);
