...
class _DiscoverySearchScreenState extends State<DiscoverySearchScreen>{
  ...
  void _personInfo(Map<String,dynamic> p){final name=(p['displayName']??p['username']??'مستخدم').toString();final id=(p['publicId']??'—').toString();showModalBottomSheet(context:context,backgroundColor:const Color(0xFF111827),shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(24))),builder:(context){final avatar=_avatar(p);return Directionality(textDirection:TextDirection.rtl,child:Padding(padding:const EdgeInsets.all(24),child:Row(children:[CircleAvatar(radius:30,backgroundImage:avatar,child:avatar==null?const Icon(Icons.person):null),const SizedBox(width:14),Expanded(child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.start,children:[Text(name,style:const TextStyle(color:Colors.white,fontSize:18,fontWeight:FontWeight.w900)),const SizedBox(height:5),Text('ID: $id',style:const TextStyle(color:Colors.white54))]))])));});}
  Widget _personCard(Map<String,dynamic> p){final avatar=_avatar(p);return Card(color:const Color(0xFF101827),child:ListTile(onTap:()=>_personInfo(p),leading:CircleAvatar(backgroundImage:avatar,child:avatar==null?const Icon(Icons.person):null),title:Text((p['displayName']??p['username']??'مستخدم').toString(),style:const TextStyle(color:Colors.white,fontWeight:FontWeight.bold)),subtitle:Text('ID: ${p['publicId']??'—'}',style:const TextStyle(color:Colors.white54)),trailing:const Icon(Icons.chevron_left_rounded,color:Colors.white38)));}
