...
class AuthBloc extends Bloc<AuthEvent,AuthState>{
 ...
 Future<void> _onSignUpRequested(SignUpRequested event,Emitter<AuthState> emit)async{emit(AuthLoading());try{final c=await _firebaseService.signUpWithEmail(event.email,event.password);final u=c.user;if(u==null){emit(Unauthenticated());return;}await _emitUser(u,emit,seed:{..._safeProfileSeed(event.userData),'email':event.email});}catch(e){emit(AuthError(e.toString()));}}
 ...
 Future<void> _onSignOutRequested(SignOutRequested event,Emitter<AuthState> emit)async{emit(AuthLoading());try{await _firebaseService.signOut();await _storageService.removeUser();await _storageService.removeToken();_webConfirmationResult=null;emit(Unauthenticated());}catch(e){emit(AuthError(e.toString()));}}
}
