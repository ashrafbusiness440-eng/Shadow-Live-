...
class AuthBloc extends Bloc<AuthEvent,AuthState>{
 ...
 Future<void> _onSignUpRequested(SignUpRequested event,Emitter<AuthState> emit)async{emit(AuthLoading());try{final c=await _firebaseService.signUpWithEmail(event.email,event.password);final u=c.user;if(u==null){emit(Unauthenticated());return;}await _sendVerification(u);emit(EmailVerificationRequired(u));}catch(e){emit(AuthError(e.toString()));}}
