...
class _ProfileSetupScreenState extends State<ProfileSetupScreen> {
  ...
  @override void dispose(){_displayNameController.dispose();_bioController.dispose();super.dispose();}
  String? _validateName(String raw){final s=raw.trim();if(s.isEmpty)return 'أدخل الاسم الظاهر';if(s.length<3)return 'الاسم الظاهر يجب أن يتكون من 3 أحرف على الأقل.';if(!RegExp(r'^[\p{L}]',unicode:true).hasMatch(s))return 'الاسم الظاهر يجب أن يبدأ بحرف، ولا يمكن أن يبدأ برقم.';return null;}
  Future<void> _pickPhone() async {final x=await _imagePicker.pickImage(source:ImageSource.gallery,imageQuality:25,maxWidth:512,maxHeight:512,requestFullMetadata:false);if(x==null)return;final b=await x.readAsBytes();if(!mounted)return;setState((){_pickedImageBytes=b;_selectedAvatarAsset=null;});if(Navigator.canPop(context))Navigator.pop(context);}
  Future<void> _showImages() async {await showModalBottomSheet<void>(context:context,backgroundColor:const Color(0xFF08111F),shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(28))),builder:(sheetContext)=>Directionality(textDirection:TextDirection.rtl,child:SafeArea(child:Padding(padding:const EdgeInsets.all(20),child:Column(mainAxisSize:MainAxisSize.min,children:[const Text('اختر صورة الحساب',style:TextStyle(color:Colors.white,fontSize:22,fontWeight:FontWeight.w900)),const SizedBox(height:18),Wrap(spacing:12,runSpacing:12,children:_avatars.map((a)=>GestureDetector(onTap:(){setState((){_selectedAvatarAsset=a;_pickedImageBytes=null;});Navigator.pop(sheetContext);},child:CircleAvatar(radius:38,backgroundImage:AssetImage(a)))).toList()),const SizedBox(height:20),SizedBox(width:double.infinity,child:OutlinedButton.icon(onPressed:_pickPhone,icon:const Icon(Icons.photo_library_rounded,color:Color(0xFFFFD54A)),label:const Text('اختيار صورة من الهاتف',style:TextStyle(color:Colors.white))))])))));}
          ...
          child: Directionality(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                Center(child:Stack(clipBehavior:Clip.none,children:[Semantics(button:true,label:'تغيير صورة الحساب',child:GestureDetector(onTap:_saving?null:_showImages,child:Container(width:132,height:132,padding:const EdgeInsets.all(3),decoration:const BoxDecoration(shape:BoxShape.circle,gradient:LinearGradient(colors:[Color(0xFF8A00FF),Color(0xFFFF00D4)])),child:ClipOval(child:_pickedImageBytes!=null?Image.memory(_pickedImageBytes!,fit:BoxFit.cover):Image.asset(_selectedAvatarAsset!,fit:BoxFit.cover))))),Positioned(left:-4,bottom:2,child:Semantics(button:true,label:'تغيير صورة الحساب',child:GestureDetector(onTap:_saving?null:_showImages,child:Container(width:44,height:44,decoration:BoxDecoration(shape:BoxShape.circle,color:const Color(0xFF171D31),border:Border.all(color:const Color(0xFFB84CFF))),child:const Icon(Icons.camera_alt_rounded,color:Colors.white)))))])),
  ...
  Widget _genderBtn(String value,IconData icon){final selected=_gender==value;return OutlinedButton.icon(onPressed:_saving?null:()=>setState((){_gender=value;_selectedAvatarAsset=value=='ذكر'?_male.first:_female.first;_pickedImageBytes=null;}),icon:Icon(icon,color:selected?const Color(0xFFFFD54A):Colors.white60),label:Text(value,style:TextStyle(color:selected?Colors.white:Colors.white60)),style:OutlinedButton.styleFrom(padding:const EdgeInsets.symmetric(vertical:15),backgroundColor:selected?const Color(0x332B1A6B):const Color(0xFF0C1728),side:BorderSide(color:selected?const Color(0xFF8A5CFF):Colors.white12)));}
