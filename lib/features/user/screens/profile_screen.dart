...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: const Color(0xFF05060D), body: SafeArea(child: user == null ? _guest() : StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(
      ...
      builder:(context,snapshot){
        ...
        return ListView(padding:EdgeInsets.zero,children:[
          ...
          const SizedBox(height:8),Padding(padding:const EdgeInsets.symmetric(horizontal:24),child:Text(bio,textAlign:TextAlign.center,style:const TextStyle(color:Colors.white70))),const SizedBox(height:18),
          Padding(padding:const EdgeInsets.symmetric(horizontal:16),child:Row(children:[Expanded(child:_button(Icons.edit_rounded,'تعديل الملف',(){})),const SizedBox(width:10),Expanded(child:_button(Icons.workspace_premium_rounded,'VIP',(){})),const SizedBox(width:10),Expanded(child:_button(Icons.apartment_rounded,'الوكالة',(){}))])),
          const SizedBox(height:16),_stats(d),const SizedBox(height:16),Padding(padding:const EdgeInsets.symmetric(horizontal:16),child:Row(children:[Expanded(child:_quick(Icons.account_balance_wallet_rounded,'محفظتي',()=>NavigationService.navigateTo(AppRoutes.wallet))),const SizedBox(width:10),Expanded(child:_quick(Icons.add_card_rounded,'شحن',()=>NavigationService.navigateTo(AppRoutes.recharge))),const SizedBox(width:10),Expanded(child:_quick(Icons.card_giftcard_rounded,'الهدايا',(){}))])),const SizedBox(height:28),
        ]);
      },
    ))));
  }
  ...
  Widget _coverAndAvatar(String cover,String photo,String asset)=>SizedBox(height:210,child:Stack(clipBehavior:Clip.none,children:[Positioned.fill(child:cover.isEmpty?Image.asset('assets/images/profile/default_profile_cover.png',fit:BoxFit.cover):Image.network(cover,fit:BoxFit.cover,errorBuilder:(_,__,___)=>Image.asset('assets/images/profile/default_profile_cover.png',fit:BoxFit.cover))),Positioned(bottom:-42,left:0,right:0,child:Center(child:Container(padding:const EdgeInsets.all(4),decoration:const BoxDecoration(shape:BoxShape.circle,gradient:LinearGradient(colors:[Color(0xFFFFC84A),Color(0xFF8A3DFF)])),child:CircleAvatar(radius:48,backgroundColor:const Color(0xFF151526),backgroundImage:photo.isNotEmpty?NetworkImage(photo):(asset.isNotEmpty?AssetImage(asset) as ImageProvider:null),child:photo.isEmpty&&asset.isEmpty?const Icon(Icons.person_rounded,size:52,color:Colors.white70):null))))]));
  Widget _genderBadge(String gender){final male=gender=='ذكر';return Text(male?'♂':'♀',style:TextStyle(color:male?const Color(0xFF42A5F5):const Color(0xFFFF69B4),fontSize:25,fontWeight:FontWeight.w900,height:1));}
