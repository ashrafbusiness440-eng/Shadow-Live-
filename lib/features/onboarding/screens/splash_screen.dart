...
class _SplashScreenState extends State<SplashScreen>{
 ...
 Future<String> _destinationAfterSplash()async{
  ...
  try{
   final snapshot=await FirebaseFirestore.instance.collection('users').doc(user.uid).get().timeout(const Duration(seconds:10));final data=snapshot.data();
   if(data==null)return AppRoutes.profileSetup;
   if(data['setupComplete']==true)return AppRoutes.main;
   final step=(data['setupStep']??'profile').toString();
   switch(step){
