...
class _RechargeScreenState extends State<RechargeScreen> {
  ...
  Future<void> _ensurePassword() async {
    ...
    try {
      ...
      if (mounted) _msg('تعذر قراءة إعدادات محفظة الألماس: $e');
    }
  }
  ...
  Widget _recipientCard(Map<String, dynamic> u) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF080A14), borderRadius: BorderRadius.circular(14)),
      child: Row(children: [CircleAvatar(radius: 24, backgroundImage: url.isNotEmpty ? NetworkImage(url) : null, child: url.isEmpty ? const Icon(Icons.person) : null), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800)), Text('ID: $id', style: const TextStyle(color: Colors.white54))]))]),
    );
  }
