...
export default async function handler(req,res){
 ...
 try{
  ...
  const result=await db.runTransaction(async tx=>{
   const opRef=db.collection("gift_operations").doc(key),senderRef=db.collection("users").doc(decoded.uid),receiverRef=db.collection("users").doc(receiverId),giftRef=db.collection("gifts").doc(giftId),conversationRef=db.collection("conversations").doc(conversationId);
   const [op,sender,receiver,gift,conversation]=await Promise.all([tx.get(opRef),tx.get(senderRef),tx.get(receiverRef),tx.get(giftRef),tx.get(conversationRef)]);
 ...
 }catch(e){const raw=e?.message||"server_error",known=["not_found","invalid_conversation","gift_inactive","invalid_gift_price","insufficient_balance"],code=known.includes(raw)?raw:"server_"+phase+"_failed",status=raw==="not_found"?404:["invalid_conversation","gift_inactive","invalid_gift_price","insufficient_balance"].includes(raw)?409:500;return out(res,status,{ok:false,code});}
}