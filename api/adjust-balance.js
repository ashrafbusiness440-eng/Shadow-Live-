...
export default async function handler(req,res){
 ...
 try{
  ...
  const canAdjust=actorSnap.exists&&(actor.role==="owner"||(actor.adminEnabled===true&&caps.includes("adjustBalances")));
  if(!canAdjust)return out(res,403,{ok:false,code:"forbidden"});
  ...
  const result=await db.runTransaction(async tx=>{
   ...
   if(op.exists)return {ok:true,code:"duplicate",operationId:key};const lockData=lock.exists?(lock.data()||{}):{};if(lockData.enabled===true||lockData.economyLocked===true)throw Error("emergency_locked");if(!user.exists)throw Error("not_found");
