...
const result = await db.runTransaction(async (tx) => {
  ...
  const roomDailyUserRef = roomDailyRef.collection("users").doc(senderUid);
  const roomWeeklyUserRef = roomWeeklyRef.collection("users").doc(senderUid);
  const roomMonthlyUserRef = roomMonthlyRef.collection("users").doc(senderUid);
